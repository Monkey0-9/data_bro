...
Goal: Ship a functional end-to-end trading intelligence platform for CME ES/NQ futures ΓÇö from raw tick ingestion through RL signal generation to a spatial 3D browser UI ΓÇö validated by prop desk quant traders.
...
**Goal:** Real-time CME MDP 3.0 market data flowing reliably through Rust decoder ΓåÆ Flink topology ΓåÆ QuestDB. The hot path proves sub-millisecond latency. No analytics yet ΓÇö pure data infrastructure.
...
2. End-to-end latency from CME UDP packet ΓåÆ QuestDB write is < 1ms at P99 measured with hdrhistogram under simulated 100k msg/sec load
...
**Goal:** Immersive 3D spatial browser UI rendering live ES/NQ market data, RL signals, and sentiment scores. 3D order book + candlestick chart + portfolio suggestion card ΓÇö at ΓëÑ 30fps on a discrete GPU browser.
...
4. Frame rate ΓëÑ 30fps sustained over 5-minute live market data session on discrete GPU; graceful degradation to WebGL on unsupported environments confirmed
...
**Plans:**
1. End-to-end integration tests (full pipeline test: CME feed ΓåÆ Flink ΓåÆ QuestDB ΓåÆ signal services ΓåÆ WebSocket ΓåÆ browser; chaos testing with simulated feed dropout, reconnect, and gap recovery)
...
3. Observability stack (structured JSON logging ΓåÆ CloudWatch, distributed tracing with OpenTelemetry, latency dashboards with hdrhistogram P99.9, alerting on SLO breaches)
...
5. User onboarding flow (account creation ΓåÆ data consent ΓåÆ platform tour ΓåÆ first signal view; doc site with API reference for staged order endpoint)

**UI hint:** no

**Success Criteria:**
1. Full pipeline (CME feed ΓåÆ browser signal display) survives a simulated 30-second feed dropout and recovers with correct order book state within 10 seconds of feed restoration
