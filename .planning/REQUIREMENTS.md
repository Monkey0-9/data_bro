# NEXUS ΓÇö v1 Requirements

## v1 Requirements

### INGEST ΓÇö Data Ingestion & Stream Processing
...
### SIGNAL ΓÇö AI Signal Generation
...
- [ ] **SIGNAL-02**: RL signal is accompanied by a confidence score (0.0ΓÇô1.0) based on the model's output probability distribution
...
- [ ] **SIGNAL-07**: A staged order object (protobuf) is generated from the portfolio suggestion and made available via API ΓÇö NEXUS never routes, fills, or tracks this order further
...
- [ ] **UI-06**: The 3D scene renders at ΓëÑ 30 frames per second on a modern browser with a discrete GPU; degrades gracefully to WebGL 1.0 on unsupported environments
- [ ] **UI-07**: Tick data updates to the 3D scene are driven from a requestAnimationFrame loop and Jotai atoms ΓÇö NOT React state ΓÇö to prevent re-render storms
...
### AUTH ΓÇö Authentication & Access
...
### INFRA ΓÇö Infrastructure & Reliability
...
- **Internal order execution / matching engine** ΓÇö Broker-dealer regulatory burden; execution stays external
- **Compliance reporting (MiFID II reports, SEC 13F)** ΓÇö Institutional PM feature; deferred beyond V3
- **Retail consumer UI** ΓÇö Dilutes prop/quant desk focus
- **Crypto as V1 asset class** ΓÇö Institutional credibility risk; add in V2 after core validated
- **Robo-advisor / investment advice functionality** ΓÇö RIA registration required
- **Bloomberg / Refinitiv data redistribution** ΓÇö Licensing prohibits it
- **Fill confirmation tracking / order lifecycle management** ΓÇö Crosses execution boundary
